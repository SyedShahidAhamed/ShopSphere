...
- Redis-based inventory consistency
...
ShopSphere uses **Redis** for high-performance caching and distributed backend operations.

## 🚀 Redis Caching

Product data is cached using Spring Cache and Redis.

```java
@Cacheable(value = "products", key = "#id")
```

Product updates refresh the corresponding cache entry:

```java
@CachePut(value = "products", key = "#id")
```

Product deletion removes the cached entry:

```java
@CacheEvict(value = "products", key = "#id")
```

This reduces unnecessary database queries for frequently accessed products.

## ⏱️ Redis TTL

Cache entries use a configurable expiration time to prevent stale data from remaining indefinitely.

## 🗑️ Cache Invalidation

Caches are invalidated when underlying product data changes.
...
Redis is used to implement API rate limiting using atomic Redis operations and expiration.
...
Redis distributed locks are used to coordinate concurrent operations.
...
Redis is used to help maintain inventory consistency during concurrent operations.
...
- Cache eviction after stock changes
...
## ⚙️ Advanced Redis Concepts

The project includes hands-on implementation and learning of:
...
ShopSphere runs as a multi-container Docker environment with separate containers for the Spring Boot application, MySQL database, and Redis.
...
**Docker Services:**
- **ShopSphere App** — Spring Boot REST API (`:8080`)
- **MySQL** — persistent relational database (`:3306`)
...
| Redis TTL | ✅ Complete |
| Redis Cache Invalidation | ✅ Complete |
| Redis Rate Limiting | ✅ Complete |
| Redis Distributed Locks | ✅ Complete |
| Redis Inventory Consistency | ✅ Complete |
| Redis Pub/Sub | ✅ Complete |
| Redis Streams | ✅ Complete |
| Redis Consumer Groups | ✅ Complete |
| Redis Transactions | ✅ Complete |
| Redis Pipelines | ✅ Complete |
| Redis Lua Scripting | ✅ Complete |
| Redis Serialization | ✅ Complete |
| Redis Connection Pooling | ✅ Complete |
| Redis Sentinel | ✅ Complete |
| Redis Cluster / Sharding | ✅ Complete |
| Redis Persistence | ✅ Complete |
| Redis Eviction Policies | ✅ Complete |
